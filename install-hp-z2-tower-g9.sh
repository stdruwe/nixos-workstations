...
# NixOS reinstallation for HP Z2 Tower G9.
...
[[ "$#" -le 1 ]] || install_die "Usage: sudo ./install-hp-z2-tower-g9.sh [target-disk]"
[[ "$EUID" -eq 0 ]] || install_die "Run as root: sudo ./install-hp-z2-tower-g9.sh [target-disk]"
[[ -d /sys/firmware/efi/efivars ]] || install_die "The installation medium was not booted in UEFI mode."

for command in \
  nix nix-instantiate nixos-install nixos-enter lsblk mountpoint readlink \
...
WORKDIR="$(mktemp -d /tmp/nixos-hp-z2-install.XXXXXX)"
